...
const createChart = (settings: any) => {
  ...
  categoryAxis.title.text = 'AI score buckets'
  ...
  categoryAxis.adapter.add('getTooltipText', (text: any) => `${text - 10} to ${text}`)
  ...
  if (settings.winrate) {
    ...
    winrateAxis.title.fill = am4core.color(theme.colors.black)
    ...
    winrateAxis.renderer.labels.template.adapter.add('text', (text: any) => `${text}%`)
  ...
  irrAxis.renderer.labels.template.adapter.add('text', (text: any) => `${text}%`)
  ...
  columnTemplate.adapter.add('fillOpacity', (_: any, target: any) => {
    if (target.dataItem) {
      const value = Number(target.dataItem.categoryX)
      let opacity = 1
  ...
  if (settings.winrate) {
    ...
    winrateSeries.stroke = theme.colors.primary
    ...
    winrateSeries.tooltip.background.stroke = am4core.color(theme.colors.primary)
    winrateSeries.tooltip.background.strokeWidth = 2
    winrateSeries.tooltip.label.fill = am4core.color(theme.colors.primary)
...
const BarChart = ({ amCharts4Loaded, ...settings }: any) => {
  if (!amCharts4Loaded) return null

  let chart: any
  useEffect(() => {
    chart = createChart(settings)
